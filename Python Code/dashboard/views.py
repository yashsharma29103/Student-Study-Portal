...
def homework(request):
    ...
    if len(homework)==0:
        ...
        homework_done = False
    context = { 'homework':homework , 'hm_done':homework_done, 'form':form}
...
@login_required
def update_todos(request, pk=None):
...
def dictionary(request):
    if request.method == "POST":
        form = DashboardForm(request.POST)
        text = request.POST['text']
        url = 'https://api.dictionaryapi.dev/api/v2/enteries/en_US/'+text
        r = requests.get(url)
        answer = r.json()
        try:
            ...
            if 'input' in request.POST:
                ...
                if input and int(input)>=0:
                    ...
                    if first=='kg' and second=='pound':
                        answer = f'{input} kg = { int(input)*2.26462 } pound'
                    context = {
                        'form':form,
                        'm_form':measurementform,
                        'input':True,
...
@login_required
def profile(request):
    homework = Homework.objects.filter(is_finished=False,user=request.use)
    todo = Todo.objects.filter(is_finished=False,user=request.use)
    if len(homework==0):
        homework_done = True
    else:
        homework_done = False
    ...
    return redirect(request, 'dashboard/profile.html', context)
